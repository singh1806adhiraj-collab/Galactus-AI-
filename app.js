/* Galactus AI - Express App (testable, no listen) */
...
// Debug cookie parsing middleware (kept out of the test env for clean output)
if (!isTest) {
...
initDatabase().then(() => {
  if (!isTest) console.log('=== DATABASE INITIALIZED ===');
}).catch(err => {
...
// 404 handler
app.use((req, res) => {
  if (!isTest) console.log('404 NOT FOUND:', req.method, req.path);
